...
class GlobalAction extends AdministratorAction {
	...
	public function testSendEmail(){
     ...
     public function configdict(){
         ...
         $data = D('Datadict')->getAllData($_GET['pid']);
         //dump($data);
         if ( $_GET['pid'] == 0 ) {
            $this->assign('back_id', '0');
        }else {
            $back_data = D('Datadict')->where('id = '.$_GET['pid'])->find();
            $this->assign('uni_code', $back_data['uni_code']);
            $this->assign('back_id', $back_data['pid']);
        }
     ...
     public function addData(){
        $data = D("DataDict")->where("id = ".$_GET['pid'])->getField("name");
        if($data == "认证类型")$flag = 1;
        else $flag = 0;
        
        $this->assign("flagsign",$flag);
        $this->assign('pid', intval($_GET['pid']));
        $this->assign('uni_code', $_GET['uni_code']);
        $this->display('editData');
     }
     
     public function editData(){
        $_GET['data_id'] = intval($_GET['data_id']);
        $area = D('Datadict')->where('id='.$_GET['data_id'])->find();
        
        $area['data_id'] = $_GET['data_id'];
    ...
    public function doAddData() {
        ...
        $_POST['uni_code']   = $_POST['uni_code'];
        $_POST['can_del']   = $_POST['can_del'];
        $num = D('Datadict')->where("pid = ".$_POST['pid'])->count()+1;
        if($_POST['pid']!=0){
            $_POST['uni_code'] = $_POST['uni_code'].str_pad($num,3,"0",STR_PAD_LEFT);
        }else{
            $_POST['uni_code'] = str_pad($num,3,"0",STR_PAD_LEFT);
        }
        
        if(isset($_POST['verify_char'])){
            $_POST['index_char']   = $_POST['verify_char'];
        }else{
             $_POST['index_char'] = $_POST['uni_code'];
        }
        
        $_POST['icon_url']=$_POST['attach'];
        ...
        F('Cache_PostData',null);
        echo ($res = D('Datadict')->add($_POST)) ? $res : '0';
    }
    ...
    public function doEditData() {
        ...
        $_LOG['type'] = '3';
        $data[] = '全局 -数据字典';
        $data[] =  D('Datadict')->where( array( 'id'=>intval($_POST['id']) ) )->find();
        if(!$_POST['verify_char']){
            $_POST['verify_char'] = $data['1']['index_char'];
        }
        $data[] = $_POST;
        ...
        F('Cache_PostData',null);
        echo D('Datadict')->where('`id`='.$_POST['id'])->setField(array('name','can_del','icon_url','index_char'), array($_POST['name'],$_POST['can_del'],$attach,$_POST['verify_char'])) ? '1' : '0';
    }
    
    public function canDelete($param){
        foreach ($param as $key => $value) {
            $data = M('Datadict')->where('id = '.$value)->find();
            if(!$data['can_del']||$data['pid']==0)
                return false;
        }
        return true;
    }
    ...
    public function doDeleteData() {
        ...
        if (empty($_POST['ids'])||!$this->canDelete($_POST['ids'])) {
            echo 0;
            return ;
        }
        ...
        $_LOG['type'] = '2';
        $data[] = '全局 -数据字典';
        $data[] =  D('Datadict')->where( $map )->findall();
        $_LOG['data'] = serialize($data);
        $_LOG['ctime'] = time();
        M('AdminLog')->add($_LOG);
        ...
        echo D('Datadict')->where($map)->delete() ? '1' : '0';
    }


    public function Indentify(){
        $verichar = $_POST['verify_char'];
        $aid = $_POST['area_id'];
        if($res = D('Datadict')->where("index_char = '".$verichar."' and id!=".$aid)->count()){
            echo $res;
        }else{
            echo 0;
        }
