...
class UserAction extends AdministratorAction {
    ...
    public function user() {
        ...
        $map['deputyoriid'] = array('exp','IS NULL');
        
        $order = "is_active ASC";
    	$dao = D('User', 'home');
    	$res = $dao->getUserList($map, true, true,'*', $order);
        //dump($res);
...
//        $res['data']=list_sort_by($res['data'],'followerCount','desc');
//        dump($res);
    	$this->assign($res);
        $this->display();
    }
	
	//组织管理 @author:jianjungki @time:2012.10.13
    public function orguser() {
    
        $map['deputyoriid'] = array('exp','IS NOT NULL');
        ...
        $map['uid'] = array('IN', $uids);
        $loginData = M('login')->field('uid, oauth_token, oauth_token_secret')->where($map)->findAll();
        //$verifyData = M('user_verified')->where($map)->findAll();
        foreach($res['data'] as &$value) {
            foreach($loginData as $val) {
                if($value['uid'] == $val['uid']) {
                    $value['oauth_token'] = $val['oauth_token'];
            ...
            /*foreach($loginData as $val) {
                if($value['uid'] == $val['uid']) {
                    ...
                    $value['realname'] = $val['realname'];
                    $value['reason'] = $val['reason'];
                    $value['info'] = $val['info'];
                    $value['verified'] = $val['oauth_token'];
                    $value['attachment'] = $val['oauth_token_secret'];
                }
            }*/
        }
//        $res['data']=list_sort_by($res['data'],'followerCount','desc');
//        dump($res);
    ...
    public function editOrg() {
        ...
        $user_credit = $credit->getUserCredit($map['uid']);
        $map['uid'] = $_GET['uid'];
        $verifyData = M('user_verified')->where($map)->find();
        if(!empty($verifyData)){
            $attach = D('attach')->where('id ='.$verifyData['attachment'])->find();
            $verifyData['attachment'] = $attach['name'];
            $verifyData['attach_id'] = $attach['id'];
        }
		$orginfo = D('org_info')->where("orgid={$user['deputyoriid']}")->select();
		$user['unitFullName'] = $orginfo[0]['unitFullName'];
		$user['telephone'] = $orginfo[0]['telephone'];
		$user['username'] = $orginfo[0]['username'];
        //dump($verifyData);
    ...
    public function doSearchUser() {
        ...
        $type = ($_GET['utype'] == 'orguser') ? 'orguser' : 'personal';
        if($type=='orguser')
			$map['deputyoriid'] = array("exp","IS NOT NULL");
		else
			$map['deputyoriid'] = array("exp","IS NULL");
     ...
     */
     public function doEditOrgUser() {
        ...
        // if ( mb_strlen($_POST['unitFullName'],'UTF8') > 50 ) {
            // $this->error('组织全称不能超过50个字符');
        // }
		
		 if (!isLegalUsername($_POST['unitFullName'],2,50)) {
			$this->error('组织全称格式为2-50位的中英文、数字、下划线、中划线');
		 }else if (checkKeyWord($_POST['unitFullName'])) {
			$this->error('简称含有敏感词');
		}
		 
		 if (!isLegalUsername($_POST['uname'])) {
			$this->error('组织简称格式为2-10位的中英文、数字、下划线、中划线');
		 }else if (checkKeyWord($_POST['uname'])) {
			$this->error('简称含有敏感词');
		}
		...
		if ( !isLegalUsername( t($_POST['username']),1,10 ) ) {
            $this->error('负责人姓名在1-10个字以内');
        }
		
		//保存组织资料
		$infokey             = array('unitShortName','unitFullName','username','telephone');
        $infovalue           = array(escape(h(t($_POST['uname']))), $_POST['unitFullName'], t($_POST['username']),$_POST['telephone']);
		
		$res = D('OrgUser','home')->where("orgid={$_POST['deputyoriid']}")->setField($infokey, $infovalue);
        ...
        if($_POST['is_active'])
            $_POST['complete']   = 0; //注册操作完成标志
        else
            $_POST['complete']   = 1; //注册操作完成标志
        //保存修改
        $key             = array('email','uname','sex','is_active','domain','complete');
        $value           = array($_POST['email'], escape(h(t($_POST['uname']))), intval($_POST['sex']), intval($_POST['is_active']),h($_POST['domain']),$_POST['complete']);
        if ( !empty($_POST['password']) ) {
            $key[]       = 'password';
            $value[]     = md5($_POST['password']);
        }
        ...
        if($userinfo['deputyoriid']&&$userinfo['is_active']==0&&$_POST['is_active']==1){
            $activate_url  = service('Validation')->addValidation($userinfo['uid'], '', U('home/Public/doOrgActivate'), 'register_activate', serialize(''));
            //设置邮件模板
        $body = <<<EOD
感谢您注册天工社区!<br>
...
EOD;
        ...
        $email_sent = service('Mail')->send_email($userinfo['email'], "激活{$userinfo['uname']}帐号",$body);
        M('user_verified')->where("uid = ".$_POST['uid'])->setField("verified",'1');
        //dump(M('user_verified'));
       }
        ...
        $this->assign('jumpUrl', U('admin/User/orguser'));
        $this->success('保存成功');
    }
    ...
    public function doAddOrgUser() {
        ...
        if ( !isLegalUsername( t($_POST['username']) ) ) {
            $this->error('负责人姓名在2-10个字以内');
        }
        if ( ! isEmailAvailable($_POST['email']) ) {
            $this->error('Email已经被使用，请重新输入');
        }
        ...
        //注册用户
        $_POST['uname']     = escape(h(t($_POST['uname'])));
        $_POST['unitFullName']     = escape(h(t($_POST['unitFullName'])));
        $_POST['unitShortName']     = escape(h(t($_POST['uname'])));
        $_POST['username']     = escape(h(t($_POST['username'])));
        $_POST['password']  = md5($_POST['password']);
        $_POST['domain']    = h($_POST['domain']);
        $_POST['ctime']     = time();
        $_POST['is_active'] = intval($_POST['is_active']);
        ...
        $orgid = D('OrgUser','home')->add($_POST);
        $_POST['deputyoriid'] = $orgid;
        $uid = M('user')->add($_POST);
     ...
     */ 
     public function GetVerified(){
         $data = D("VerifiedLog",'home')->AdminListLog();
         foreach($data['data'] as $key=>$value){
             $data['data'][$key]['location']      =   getLocation($value['province'],$value['city']);
         }
     ...
     */ 
     public function ajaxPassed(){
         $data = D("VerifiedLog",'home')->VerifyPassed($_POST['logidstr']);
         if($data){
             echo 1;
         }else{
             echo 0;
    ...
    public function doDeleteOrgUser() {
    	...
    	$user = D('User', 'home');
		$user->startTrans();
		$res = $user->deleteUser($_POST['uid']);
		$next = D('OrgUser', 'home')->where(array("orgid"=>$_POST['deputyoriid']))->delete();
		
    	if($res&&$next) { $user->commit();echo 1;		  }
    	else 	 {$user->rollback();echo 0; return ;}
    }
     ...
     */
     function DelLog(){
         $res = D("VerifiedLog",'home')->DelLog($_POST['logidstr']);
         echo $res;
     }
    ...
    public function doChangeRecommend(){
            $user['uid'] = array( 'in',$_REQUEST['id']);        //要推荐的id.
            $act  = $_REQUEST['type'];  //推荐动作
             $is_active = D('User', 'home')->where("uid = ".$_REQUEST['id'])->getField("is_active");
            if(!$is_active){
                echo 2;return;
            }
