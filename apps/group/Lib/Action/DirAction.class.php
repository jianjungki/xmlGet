...
class DirAction extends BaseAction
{
	...
	function index() {
		...
		//判断圈子、项目、组织
		$groupId = explode(',', $this->group_ids['group']);
		$projectId = explode(',', $this->group_ids['project']);
		$orgId = explode(',', $this->group_ids['org']);
		
		if (in_array($_GET['gid'], $projectId)) {
			$dirConfig = $this->projectConfig;
		}else if (in_array($_GET['gid'], $orgId)) {
			$dirConfig = $this->orgConfig;
		}else if (in_array($_GET['gid'], $groupId)) {
			$dirConfig = $this->config;
		}
		...
		$this->assign('usedRate', ($usedSpace/($dirConfig['spaceSize']*1024*1024)));  //空间使用率
	...
	function upload() {
		...
		if($usedSpace >= $dirConfig['spaceSize']*1024*1024) {
			if ($_GET['ajax'] == 1) {
		...
		if(isset($_POST['uploadsubmit']) || $_GET['ajax'] == 1) {
			//上传参数
			$upload['max_size']   = $dirConfig['simpleFileSize']*1024*1024;
			$upload['allow_exts'] = str_replace('|', ',', $dirConfig['uploadFileType']);
			// $upload['allow_exts'] = str_replace('|',',',$dirConfig['uploadFileType']);
		...
		$this->assign('uploadType',str_replace('|', ',', $dirConfig['uploadFileType']));
