...
class AdminAction extends AdministratorAction {
	...
	public function _initialize() {
		...
		if (!empty($_GET['gtype'])) {
			S('admin_group_type_' . $this->mid,null);	
			S('admin_group_type_' . $this->mid,$_GET['gtype'],3600);
		}
		$this->grouptype = S('admin_group_type_' . $this->mid);
		$this->assign('gtype',$this->grouptype);
		switch($this->grouptype){
			case 'project' : $this->typenum = 1; break;
			case 'org' : $this->typenum = 2; break;
			default : $this->typenum = 0; break;
		}
	}
	...
	public function index()
	{
		if (isset ( $_POST ['editSubmit'] ) == 'do') {
			array_map ('h', $_POST);
			$res = model ( 'Xdata' )->lput ( $this->grouptype, $_POST );
			if ($res) {
				$this->success ( '保存成功' );
			} else {
				$this->error ( '保存失败' );
		...
		$setting = model ( 'Xdata' )->lget ( $this->grouptype );
	...
	{//为使搜索条件在分页时也有效，将搜索条件记录到SESSION中
		...
		if ('group' == $type) {
			// 圈子
			$condition[] = 'status=1 AND grouptype = ' . $this->typenum;
			!$order && $order = 'ctime DESC';
			$list = D('Group')-> getGroupList(1, $condition, null, $order, $limit, 0);
		} else if ('weibo' == $type) {
			// 圈聊
			$condition[] = 'gw.isdel=0 AND grouptype = ' . $this->typenum;
			!$order && $order = 'ctime DESC';
			//$list = D('GroupWeibo')->where(implode(' AND ', $condition))->order($order)->findPage($limit);
			$list = M() ->Table('`'.C('DB_PREFIX').'group_weibo` AS gw '
	  						  . " LEFT JOIN " . C('DB_PREFIX') . "group AS g ON g.id = gw.gid ")
						->field('gw.*,g.grouptype')
						->where(implode(' AND ', $condition))->order($order)->findPage($limit);
		} else if ('topic' == $type) {
			// 帖子	
			$list = D('Topic')->getTopicList(1, $condition, null, $order, $limit, 0, $this->typenum);
		} else if ('post' == $type) {
			// 回帖
   			$list = D('Post')->getPostList(1, $condition, null, $order, $limit, 0, $this->typenum);
	    	/*
	    	 * 缓存帖子标题
	    	 */
			...
			// 文件
   			$list = D('Dir')->getFileList(1, $condition, null, $order, $limit, 0, $this->typenum);
		} /* else if ($type == 'album') {
   				// 相册
	...
	public function audit() {
		$audit_list = D ( 'Group' )->field ( 'id,uid,name,intro,logo,ctime' )->where ( "status=0 AND is_del=0 AND grouptype={$this->typenum}" )->order ( 'ctime DESC' )->findPage ();
	...
	public function doAudit() {
		...
		if ($res) {
			...
			foreach ( $groups as $v ) {
				$type = $this->getGroupType($v ['id']);
				$notify_data = array ('title' => $v ['name'], 'group_id' => $v ['id'] );
				$notify_dao->sendIn ( $v ['uid'], $type . '_audit', $notify_data );
			}
		} else {
			echo 0;
		}
	...
	public function doDismissed()
	{
		...
		if ($res) {
			...
			if ($_POST ['reason']) {
				...
				$notify_template = '_delaudit';
			} else {
				$notify_template = '_del';
			}
			$map ['id'] = array ('in', $_POST ['gid'] );
			$groups = D ( 'Group' )->field ( 'id,uid,name' )->where ( $map )->findAll ();
			$notify_dao = service ( 'Notify' );
			foreach ( $groups as $v ) {
				$type = $this->getGroupType($v ['id']);
				$notify_data = array ('title' => $v ['name'], 'reason' => $reason );
				$notify_dao->sendIn ( $v ['uid'], $type . $notify_template, $notify_data );
			}
	 ...
	 * lianggh
	 */
	function getGroupType($gid) {
	  
		$topic_type = 'group';
		$group_ids = $this->flashGroupCache();
		$gids = split(',', $group_ids['project']);
		if (in_array($gid, $gids)) {
			$topic_type = 'group_project';
		}
		$gids = split(',', $group_ids['org']);
		if (in_array($gid, $gids)) {
			$topic_type = 'group_org';
		}
		return $topic_type;
	...
	function recom() {
		...
		$isrecom = ! empty ( $_POST ['isrecom'] ) ? trim ( $_POST ['isrecom'] ) : 'isrecom';
		$data['isrecom'] = $isrecom;
		$res = D ('Group','group')->where ('id = ' . $id)->save($data);
		if ($res) {
			echo 1;
		}else {
			echo 0;
