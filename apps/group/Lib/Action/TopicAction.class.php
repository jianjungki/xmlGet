<?php
	class TopicAction extends BaseAction
	{
		...
		public function _initialize(){
			...
			//判断话题类型，选择圈子、项目、组织对应的标签文字
			$this->label = '帖子';
			$this->topic_type = 'group';
			$group_ids = S('Cache_GroupType');
			$gids = split(',', $group_ids['project']);
			if (in_array($this->gid, $gids)) {
				$this->label = '任务';
				$this->topic_type = 'group_project';
			}
			$gids = split(',', $group_ids['org']);
			if (in_array($this->gid, $gids)) {
				$this->topic_type = 'group_org';
			}
			
			// 判断功能是否开启
			if (!$this->groupinfo['openBlog']) {
				$this->assign('jumpUrl', U('group/Group/index', array('gid'=>$this->gid)));
				$this->error($this->label .'功能已关闭');
			}
		...
		function index() {
        	...
        	$this->setTitle($this->label ." - " . $this->groupinfo['name']);
 			$this->display();
		}
		...
		public function add()
		{
        	...
        	$this->setTitle("发新".$this->label ." - " . $this->groupinfo['name']);
			$this->display();
		}
		...
		public function doAdd()
		{
			if(isset($_POST['addsubmit']) && trim($_POST['addsubmit']) == 'do') {
				...
				if($tid = D('Topic')->add($topic)) {
					...
					//更新ts_group的帖子数量
					$data['threadcount'] = $this->topic->where('gid=' . $this->gid)->count();
					$group = D('Group','group');
					$group->where('id=' . $this->gid)->save($data);

					// 微博
					$weibo_template = 'group_post_create_weibo';
					if ($this->label == '任务') {
						$weibo_template = 'project_post_create_weibo';
					}
					$weibo_tpl_data = array('author'=>getUserName($topic['uid']),'title'=>$topic['title'],'url'=>U('group/Topic/topic',array('gid'=>$this->gid,'tid'=>$tid)));
					$weibo_tpl_data = model('Template')->parseTemplate($weibo_template, array('body'=>$weibo_tpl_data));
		...
		public function edit()
		{
			...
			if(isset($_POST['editsubmit']) && trim($_POST['editsubmit']) == 'do') {
				...
				$this->success("编辑".$this->label ."成功");
			}

			$this->assign('thread',$thread);
			$this->assign('category_list', $this->topic->categoryList($this->gid));
        	$this->setTitle("编辑".$this->label ." - " . $this->groupinfo['name']);
			$this->display();
		}
		...
		public function post()
		{
			...
			if($tid > 0) {
				...
				if (!$topic) {
					$this->error($this->label ."不存在或已被删除");
				} else if($topic['lock'] == 1) {
					$this->assign('jumpUrl', U('group/Topic/topic', array('gid'=>$this->gid, 'tid'=>$tid)));
					$this->error($this->label ."已被锁定，不可回复");
				}
				...
				if(isset($_POST['quote'])) {  //如果引用帖子
					...
					if ($post_info['uid'] != $this->mid) {
						...
						$notify_data = array (
									 	'post' 	  => $post_info['istopic'] ? "的".$this->label ."“{$topic['title']}”并回复您" : "在".$this->label ."“{$topic['title']}”中的回复",
						...
						$notify_dao->send($post_info['uid'], $this->topic_type . '_topic_quote', $notify_data, $this->mid);
						/*$message_dao = D ( 'Message' );
                        $message_data = array (
                                        'to'       =>  $post_info['uid'],
                                        'title'    => $post_info['istopic'] ? "的".$this->label ."“{$topic['title']}”并回复您" : "在".$this->label ."“{$topic['title']}”中的回复",
                                        'content' =>  $post_info['istopic'] ? "的".$this->label ."“{$topic['title']}”并回复您" : "在".$this->label ."“{$topic['title']}”中的回复".html_entity_decode($post['content'])."<a href='".U("home/group/Topic/",array("act"=>"topic","gid"=>$this->gid,"tid"=>$topic['id']))."'>去看看</a>",
                                       );
                        $message_dao->postMessage($message_data, $this->mid);*/
	    				D('GroupUserCount')->addCount($post_info['uid'], 'bbs_'.$this->topic_type);
					}
				}
				...
				if($result) {
					if ($topic['uid'] != $this->mid && $post_info['uid'] != $topic['uid']) {
						...
						$notify_dao->send($topic['uid'], $this->topic_type . '_topic_reply', $notify_data, $this->mid);
                        
						/*$message_dao = D ( 'Message' );
                        $message_data = array (
                                        'to'       =>  $topic['uid'],
                                        'title'    => $topic['title'],
                                        'content' =>  "“{$topic['title']}”中回复您".html_entity_decode($post['content'])."<a href='".U("home/group/Topic/",array("act"=>"topic","gid"=>$this->gid,"tid"=>$topic['id']))."'>去看看</a>",
                                       );
                        $message_dao->postMessage($message_data, $this->mid);*/
	    				D('GroupUserCount')->addCount($topic['uid'], 'bbs_'.$this->topic_type);
					}
		...
		public function editPost()
		{
			...
			//管理员或者帖子主人
			if(!$post) { $this->error($this->label ."回复不存在");}
        	...
        	$this->setTitle("编辑".$this->label ."回复 - " . $this->groupinfo['name']);
			$this->display();
		}
		...
		public function search() {
        	...
        	$this->setTitle("帖子".$this->label ." - " . $this->groupinfo['name']);
 			$this->display();
		}
		...
		public function topic()
		{
        	...
        	$this->setTitle("{$thread['title']} - ".$this->label ." - {$this->groupinfo['name']}");
			$this->display();
		}
			...
			if($result !== false) {
				...
				foreach ($topicInfo as $v) {
					if ($v['uid'] != $this->mid && !$v['dist']) {
						...
						service ( 'Notify' )->send($v['uid'], $this->topic_type . '_topic_dist', $notify_data, $this->mid);
	    				D('GroupUserCount')->addCount($v['uid'], 'bbs');
					}
				}

				exit(json_encode(array('flag'=>'1', 'msg'=>$this->label .'设为精华成功')));
			}else{
				exit(json_encode(array('flag'=>'0', 'msg'=>$this->label .'设为精华失败')));
			}
		}
		...
		public function undist()
		{
			...
			if($result !== false) {
				...
				exit(json_encode(array('flag'=>'1','msg'=>$this->label .'取消精华成功')));
			}else{
				exit(json_encode(array('flag'=>'0','msg'=>$this->label .'取消精华失败')));
			}
		}
			...
			if($result !== false) {
				...
				foreach ($topicInfo as $v) {
					if ($v['uid'] != $this->mid && !$v['top']) {
						...
						service ( 'Notify' )->send($v['uid'], $this->topic_type . '_topic_top', $notify_data, $this->mid);
	    				D('GroupUserCount')->addCount($v['uid'], 'bbs');
					}
				}

				exit(json_encode(array('flag'=>'1','msg'=>$this->label .'置顶成功')));
			}else{
				exit(json_encode(array('flag'=>'0','msg'=>$this->label .'置顶失败')));
			}
		}
		...
		protected function _setOperationLog($operation, &$post_info)
		{
            $content =  '把 ' . getUserSpace($post_info['uid'], 'fn', '_blank', '@' . getUserName($post_info['uid']))
						 . " 的".$this->label."“<a href=\"" . U('group/Topic/topic', array('gid'=>$this->gid, 'tid'=>$post_info['id'])) . '" target="_blank">'
