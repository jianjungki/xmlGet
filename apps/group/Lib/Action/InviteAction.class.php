...
class InviteAction extends BaseAction {
	...
	public function create()
	{
		...
		if($_POST['sendsubmit']) {
			...
			$message_data['title']   = "邀您加入".getLabel($this->gid). $this->groupinfo['name'];
			$url = '<a href="' . U('group/Group/index', array('gid'=>$this->gid)).'" target="_blank">去看看</a>';
			$message_data['content'] = "你好，诚邀您加入“{$this->groupinfo['name']}” ".getLabel($this->gid)."，点击   " . $url . '进入。';
            ...
            if (count($toUserIds) > 0) {
            	...
            	$this->error('邀请失败,您的好友可能已经加入了该'.getLabel($this->gid).'！');
            	//echo -1;
            }
			exit;
		}
