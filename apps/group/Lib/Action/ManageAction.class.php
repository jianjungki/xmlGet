...
class ManageAction extends BaseAction
{
	...
	public function index()
	{
		if(isset($_POST['editsubmit'])) {
			...
			if (!$group['name'])
				$this->error($this->label . '名称不能为空');
			else if (get_str_length($group['name']) > 30)
				$this->error($this->label . '名称不能超过30个字');

			if (D('Category')->getField('id', 'name=' . $group['name'])) {
				$this->error('请选择' . $this->label . '分类');
			}
			if (get_str_length($group['intro']) > 2000) {
				$this->error($this->label . '简介请不要超过2000个字');
			}
			if (($this->label=='圈子')&&(!preg_replace("/[,\s]*/i", '', $_POST['tags']) || count(array_filter(explode(',', $_POST['tags']))) > 5)) {
			...
			if ($res !== false) {
				D('Log')->writeLog($this->gid, $this->mid, '修改' . $this->label . '基本信息', 'setting');
		...
		//组织分类
		$orginfo['org_category']  = M('org_info')->where('orgid=' . $this->orgid)->getField('category');
	...
	public function privacy()
	{
		...
		if(isset($_POST['editsubmit'])){
			...
			if ($res !== false) {
				D('Log')->writeLog($this->gid, $this->mid, '修改' . $this->label . '访问权限', 'setting');
	...
	{
		$type = (isset($_GET['type']) && in_array($_GET['type'], array('manage','apply','invite')))?$_GET['type']:'';
		if ('apply' == $type) {
		...
		if ('invite' == $type) {
			//$memberlist = D('Member')->where("gid={$this->gid} AND level=0")->order('level ASC')->findPage();
			$memberlist = D('group_invite_verify')->distinct(true)->where("gid={$this->gid} AND is_used=0")->order('invite_id desc')->group('gid,uid')->findPage();
			$type = 'invite';
		}
		if ('manage' == $type || (!$memberlist['data'] && 'apply' != $_GET['type'] && 'invite' != $_GET['type'])) {
			$memberlist = D('Member')->where("gid={$this->gid} AND level>0")->order('level ASC')->findPage();
			$type = 'manage';
		}
		...
		switch ($_POST['op'])
		{
			...
			case 'out':     // 剔除会员
				...
				if($batch) {
					...
					if ($res) {
						$content = '将用户 ' . getUserSpace($this->uid, 'fn', '_blank', '@' . getUserName($this->uid)) . '踢出' . $this->label;
						// 被拒绝加入不扣积分
	...
	public function sendMessage() {
		$uid = $_POST['uid'];
		 $data['to'] = $uid;
		$data['title']   = "邀您加入".getLabel($this->gid). $this->groupinfo['name'];
		$url = '<a href="' . U('group/Group/index', array('gid'=>$this->gid)).'" target="_blank">去看看</a>';
		$data['content'] = "你好，诚邀您加入“{$this->groupinfo['name']}” ".getLabel($this->gid)."，点击   " . $url . '进入。';
