...
class GroupAction extends BaseAction
{
    ...
    public function index()
    {
		...
		$uids = D('Member','group')->field('uid')->where('gid='.$this->gid.' and (level=1 or level=2)')->select();
		if (in_array($this->mid, getSubByKey($uids,'uid'))) {
    ...
    public function  joinGroup()
    {
        if (isset($_POST['addsubmit'])) {
            ...
            if ($this->is_invited) {
                ...
                $message_data['title']   = "申请加入".getLabel($this->gid).$this->groupinfo['name'];
                $message_data['content'] = "你好，请求你批准加入“{$this->groupinfo['name']}” ".getLabel($this->gid)."，点此"
    ...
    function endProject() {
        ...
        if(!iscreater($this->mid,$this->gid))  exit('你没有权限');
        D('Group')->where('id=' . $this->gid)->setField('isDone','1');
        // S('Cache_MyGroup_'.$this->mid, NULL);
        exit('1');
    }
    ...
    //删除群组对话框
    function delProjectDialog() {
    	$this->assign('gid',$this->gid);
        $this->display();
    }
	
	//结束项目对话框
    function endProjectDialog() {
