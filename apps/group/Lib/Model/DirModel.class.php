...
class DirModel extends Model {
   ...
   * getGroupList 
    *
   */
  public function getFileList($html=1,$map = null,$fields=null,$order = null,$limit = null,$isDel=0,$grouptype) {
      //处理where条件
      ...
      $result = M()->Table('`'.C('DB_PREFIX').'group_attachment` AS ga LEFT JOIN `'.C('DB_PREFIX').'attach` AS a ON a.id = ga.attachId'
	  						  . " LEFT JOIN " . C('DB_PREFIX') . "group AS g ON g.id = ga.gid "
							)
                   ->field('ga.*,g.grouptype')
                   ->where($map)
                   ->order($order)
                   ->findPage($limit);
