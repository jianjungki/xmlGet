...
class WeiboOperateModel extends GroupWeiboModel{
   ...
   public function getSavePath(){
    ...
    public function deleteMini($id, $gid, $uid){
		if(isadmin($uid, $gid)){
			$where = "weibo_id={$id} AND gid={$gid}";
		}else{
			$where = "weibo_id={$id} AND uid={$uid} AND gid={$gid}";
		}
    ...
    function getAtme($uid, $gid = null, $api = null){
    	...
    	$count_sql = "SELECT count(*) AS count FROM {$this->tablePrefix}group_weibo AS w INNER JOIN {$this->tablePrefix}group_weibo_atme AS a ON w.weibo_id = a.weibo_id
    				  WHERE a.uid = {$uid} " . ($gid ? " AND w.gid IN({$gid}) " : '')
    				  . " AND a.uid NOT IN ( SELECT b.fid FROM {$this->tablePrefix}user_blacklist AS b WHERE b.uid = {$uid} )";
    	$count = $this->query($count_sql);
    	$count = $count[0]['count'];

    	$list = $this->where(($gid ? " gid IN({$gid}) AND " : '') . "isdel=0 AND weibo_id IN (SELECT weibo_id FROM {$this->tablePrefix}group_weibo_atme WHERE uid=$uid) AND uid NOT IN (SELECT fid FROM {$this->tablePrefix}user_blacklist WHERE uid=$uid)")
    				 ->order('ctime DESC')
    				 ->findPage(10, $count);
