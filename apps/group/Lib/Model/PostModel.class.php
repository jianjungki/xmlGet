...
class PostModel extends Model {
     ...
     */
     public function getPostList($html=1,$map = null,$fields=null,$order = null,$limit = null,$isDel=0, $grouptype) {
            //处理where条件
            if(!$isDel)$map[] = 'gp.is_del=0';
            else $map[] = 'gp.is_del=1';
            ...
            //$result         = $this->where( $map )->field( $fields )->order( $order )->findPage($limit) ;
          	$result  =M() ->Table('`'.C('DB_PREFIX').'group_post` AS gp '
	  						  . " LEFT JOIN " . C('DB_PREFIX') . "group AS g ON g.id = gp.gid ")
