...
class TopicModel extends Model
{
	...
	//获取帖子列表
      ...
      * getTopicList 
     */
    public function getTopicList($html=1, $map = null, $fields=null, $order = null, $limit = null, $isDel=0, $grouptype)
    {
            //处理where条件
            if(!$isDel)$map[] = 'gt.is_del=0';
            else $map[] = 'gt.is_del=1';
			
			 if (!empty($grouptype)) {
				  $map[] = 'g.grouptype=' . $grouptype;
			  }
            
   			$map = implode(' AND ', $map);
            //连贯查询.获得数据集
            //$result  = $this->where( $map )->field( $fields )->order( $order )->findPage( $limit ) ;
			$result  =M() ->Table('`'.C('DB_PREFIX').'group_topic` AS gt '
	  						  . " LEFT JOIN " . C('DB_PREFIX') . "group AS g ON g.id = gt.gid ") 
						  ->where( $map )->field( $fields )->order( $order )->findPage( $limit ) ;
    ...
    public function getHotThread(){
		...
		$cache = $this->field('topic.id,topic.gid,topic.title,topic.dist,post.content')
                           ...
                           ->where('`group`.grouptype=0 AND `group`.brower_level=-1 AND post.istopic = 1 AND topic.is_del=0 and post.is_del=0 AND topic.replytime>' . (time()-30*24*3600))
