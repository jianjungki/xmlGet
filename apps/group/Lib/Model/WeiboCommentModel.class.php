    ...
    function doaddcomment($uid,$post,$api=false){
        ...
        if ($comment_id = $this->addcomment($data)){
            ...
            if($gtype == 0)
                ...
                $str = 'org_comment';
            // 添加统计
            D('GroupUserCount','group')->addCount($data['reply_uid'],$str);
            if($data['reply_uid'] != $miniInfo['uid']){
            	D('GroupUserCount','group')->addCount($miniInfo['uid'],$str);
            }
