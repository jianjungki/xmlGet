...
class GroupModel extends Model {
	...
	//获取我的群组  包括我管理的和加入的群组
	public function getAllMyGroup($mid, $html=0,$open = array(),$limit=false,$grouptype=0)
	{
	    ...
	    $groupList = $this->table(C('DB_PREFIX')."group_member as member left join ".C('DB_PREFIX')."group as g on g.id = member.gid")
                    	  ->field('g.id,g.uid,g.name,g.openWeibo,g.type,g.membercount,g.logo,g.cid0,g.ctime,g.status')
                    	  ->where('member.uid = '.$mid.' and member.level>0 and g.is_del = 0 and g.grouptype = '.$grouptype.' '.$openSql)
	...
	//我管理的群组
	public function mymanagegroup($mid, $html=0,$grouptype=0)
	{
		$gidarr = D('Member')->field('gid')->where('(level=1 OR level=2) AND uid='.$mid)->findAll();

		if($gidarr){
		    // if ((ACTION_NAME=='projectIndex')||isProject($_GET['gid'])) {
				// $in = 'id IN '.render_in($gidarr,'gid') . ' AND is_del=0' . ' AND deputyorgid is not NULL';
			// }else{
				// $in = 'id IN '.render_in($gidarr,'gid') . ' AND is_del=0' . ' AND deputyorgid is NULL';
			// }
			$in = 'id IN '.render_in($gidarr,'gid') . ' AND is_del=0' . ' AND grouptype = '.$grouptype;
		    $groupList = D('Group')->field('id,uid,name,type,membercount,logo,cid0,ctime,status')->where($in)->order('ctime desc')->findPage();
	...
	//我加入的群组
	public function myjoingroup($mid,$html=0,$grouptype=0)
	{
	...
	//好友加入的群
	function friendjoingroup($mid,$grouptype=0) {
		...
		if(!empty($friendlist) && is_array($friendlist)) {
			...
			// if ((ACTION_NAME=='projectIndex')||isProject($_GET['gid'])) {
				// $in = 'uid IN '.render_in($friendlist,'fuid'). ' AND deputyorgid is not NULL';
			// }else{
				// $in = 'uid IN '.render_in($friendlist,'fuid'). ' AND deputyorgid is NULL';
			// }
			$in = 'uid IN '.render_in($friendlist,'fuid'). ' AND grouptype = '.$grouptype;
			$count = D('Member')->field('count(distinct(gid)) AS count')->where($in)->find();  //显示分页总数
			if($count['count'] == 0) return '';

			$p = new Page($count['count'],10);
			$friendgroup = D('Member')->field('gid')->where($in)->group('gid')->order('ctime desc')->limit($p->firstRow.','.$p->listRows)->findAll();  //获取数据
	...
	function joinGroup($mid,$gid,$level,$incMemberCount=false,$reason='') {
		...
		if(D('Member','group')->where("uid=$mid AND gid=$gid")->order('ctime desc')->find()) exit('你已经加入过');
	...
	// 个人感兴趣的群组
	public function interestingGroup($uid, $pagesize = 4,$gid)
	{
		// 个人标签
		...
		// 管理和已经加入的群组
    	$my_group = D('Member')->field('gid')->where(($gid ?  "gid IN({$gid}) AND "  : '').'(level >= 1 AND status=1)  AND uid=' . $uid)->findAll();
    	foreach ((array)$my_group as $v) {
    		$_my_group_id[] = $v['gid'];
    	}

    	$map = 'g.status=1 AND g.is_del=0 AND g.grouptype=0';
		...
		if ($group_list['count'] < 4) {
			...
			if ($group_count['count'] < 4) {
				...
				$rand_list = $this->field('id,name,logo,membercount,ctime')
	    				 		  ->where('grouptype=0 AND status=1 AND is_del=0 ' . $gid_map)
     ...
     * @return unknown
     */
    public function getHotList($reset=false,$isProject=false)
    {
    	// 1分钟锁缓存 
    	...
    	$count['membercount'] = $hot_list = $this->field('id,id AS gid,name,logo,membercount,ctime')
    					 ->where('grouptype=0 AND status=1 AND is_del=0 ' . $gid_map)->order('membercount desc')
    					 ->findAll();
		
		//筛选圈子
		$res = $this->getProjectByMap('grouptype=0', '*', '', '', FALSE);
		$res = getSubByKey($res,'id');
	...
	function getMyJoinGroup($uid,$appid) {
	 ...
	 /**
      ...
      public function getProjectByMap($map, $field = '*', $limit = '', $order = '', $is_find_page = true) {
		$m = 'is_del=0';	
		if (isset($map)) {
			$m = $m . ' AND ' . $map;
		}	
		if ($is_find_page) {
			return $this->where($m)->field($field)->order($order)->findPage($limit);
		}else {
			return $this->where($m)->field($field)->order($order)->limit($limit)->findAll();
		}
	}
	  
	  public function end($id){
	  	//更新项目完成状态
		$data['isDone'] = D('Group')->where('gid=' . $id)->count();
		$group = D('Group','group');
		$group->where('id=' . $this->gid)->save($data);
	  }
	  
	  //判断是否项目
	public function isProject($gid) {
		$flag  = 0;
	...
	public function getMemberId($gid,$limit='') {
		$ids = D('Member','group')->field('uid')->where('gid = '.$gid)->limit($limit)->order('level,ctime')->findAll();
		return $ids;
	}
