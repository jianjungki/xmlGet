...
group_info.prototype = {
	...
	{
		var title = '圈子';
		if(v_form.action_name.value=='addPro'){
			title = '项目';
		}
		if (getLength(v_form.name.value) == 0) {
			ui.error(title+"名称不能为空");
			v_form.name.focus();
			return false;
		} else if (getLength(v_form.name.value) > 30) {
			ui.error(title+"名称不能超过30个字");
			v_form.name.focus();
			return false;
		} else if (getLength(v_form.cid0.value)== 0 && title != '项目') {
			ui.error("请选择"+title+"分类");
			v_form.cid0.focus();
			return false;
		} else if (getLength(v_form.intro.value) > 200) {
			ui.error(title+"简介不能超过200个字");
			v_form.intro.focus();
			return false;
		} else if (getLength(v_form.tags.value.replace(/,/ig,'')) == 0 && title == '圈子') {
			ui.error(title+"标签不能为空");
			v_form.tags.focus();
			return false;
		} else if (this.tag_num() >5&&title == '圈子') {
			ui.error("标签个数不能超过5个");
			return false;
