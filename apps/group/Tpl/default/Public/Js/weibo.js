...
weibo.prototype = {
	...
	transpond:function(id, gid, upcontent){
		var label ='圈子';
		$.post(U('group/Index/getLab'),{gid:gid},function(txt){
			label = txt;
			upcontent = ( upcontent== undefined ) ? 1 : 0;
			ui.box.load( U("group/WeiboOperate/transpond",["id="+id,"gid="+gid,"upcontent="+upcontent] ),{title:'转发到'+label,closeable:true});
		});
