...
//	    		preg_match_all('/#(.*)#/isU',$info['content'],$topic_arr);
    ...
    //搜索话题--TODO
    public function doSearchWithTopic($key, $type,$order) {
    	global $ts;
        if($order == "content_time"){
            $orderinfo = 'weibo_id';
        }else{
            $orderinfo = 'weibo_id DESC';
        }
    	//$key = addslashes(t($key));
    	if (!$key) {
    		$list['count'] = 0;
    	...
    	switch ($type){
    		case '':
				$list = $weiboTopicLinkModel->where("topic_id = {$topicId}".$map)->order($orderinfo)->findPage(20);
    			break;

    		case 'original':
				$list = $weiboTopicLinkModel->where("topic_id = {$topicId} AND transpond_id = 0".$map)->order($orderinfo)->findPage(20);
    			break;

    		case 'image':
				$list = $weiboTopicLinkModel->where("topic_id = {$topicId} AND type = '1'".$map)->order($orderinfo)->findPage(20);
    			break;

    		case 'music':
				$list = $weiboTopicLinkModel->where("topic_id = {$topicId} AND type = '4'".$map)->order($orderinfo)->findPage(20);
    			break;

    		case 'video':
				$list = $weiboTopicLinkModel->where("topic_id = {$topicId} AND type = '3'".$map)->order($orderinfo)->findPage(20);
    			break;

    		case 'file':
				$list = $weiboTopicLinkModel->where("topic_id = {$topicId} AND type = '5'".$map)->order($orderinfo)->findPage(20);
    			break;

    		default:
				$list = $weiboTopicLinkModel->where("topic_id = {$topicId} AND type = '{$type}'".$map)->order($orderinfo)->findPage(20);
    	}

		$weiboIdArr = getSubByKey($list['data'], 'weibo_id');
		$listMap['weibo_id'] = array('IN', $weiboIdArr);
		$data = $this->field('weibo_id')->where($listMap)->order($orderinfo)->findAll();
    ...
    function getLastWeibo(){
    	...
    	$data = $this->order("weibo_id DESC")->limit(20)->findAll();
		S('Cache_LastWeibo',$data);
		return $data;
    }
	//Topic搜索
	function doSearchTopic($map,$order,$uid,$limit = true) {
		...
		if($limit){
		    $list = $this->field('weibo_id')->where($map)->order($order)->findPage(10);
		    $weibo_id_list = getSubByKey($list['data'],'weibo_id');
		    $list['data'] = $this->getWeiboDetail($weibo_id_list);
		}else{
...
//    				 ->findPage(10, $count);
    ...
    function getWork($uid,$mywork){
    	...
    	*/
    	...
    	// $sql="SELECT wb.*,wt.work_type FROM ts_weibo wb inner join ts_work_task wt on wb.weibo_id=wt.app_id WHERE wt.app_type=1 AND wt.uid=".$uid." ".$map." order by wb.ctime desc";
		// $sql="SELECT wb.*,wt.work_type FROM ts_weibo wb inner join {$db_prefix}work_task wt on wb.weibo_id=wt.app_id WHERE wt.app_type=1 AND wt.uid=".$uid." ".$map." order by wb.ctime desc";
		// $list['data'] = M()->query($sql);
		if ($mywork=='finish') {
			
			$list['data'] = M('')->table("ts_work_finishtask")->field('task_id as weibo_id,title,content,work_type,uid as owner_id,from_id as uid,start_date as ctime,now_end_date,app_type,app_id,isdel')
			->where("uid={$uid} AND isdel=0")->order("now_end_date desc")->select();
			foreach( $list['data'] as $key=>$value){
	        	$value['is_finish'] = '1';
	        	$list['data'][$key] = $this->getOneLocation('',$value);
	        }	
			...
			$list['data'] = M('')->table("ts_work_task")->field('task_id as weibo_id,title,content,work_type,uid as owner_id,from_id as uid,start_date as ctime,app_type,app_id,isdel')
			->where("uid={$uid} {$map}  AND isdel=0")->order("start_date desc")->select();
