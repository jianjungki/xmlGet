	...
	public function getTopFollowerUser($uid = 0, $count = 10, $hide_auto_friend = null, $hide_no_avatar = null)
	{
		...
		if (($top_user = S($cache_id)) === false) {
			...
			$limit += $hide_no_avatar ? $count * 3 : $count;
               ...
               /*         switch ($utype) {
                            case "org":$where = ' WHERE `type` = 0 and fid in(select uid from ts_user where deputyoriid IS NOT NULL and deputyoriid <>"") ';
                                break;
                            case "personal":$where = ' WHERE `type` = 0  ';
                                break;
                            
                            default:$where = ' WHERE `type` = 0  ';
                                break;
                        }
               */         
                       $where = ' WHERE `type` = 0 ';
        ...
        public function getTopFollowerOrg($utype,$uid = 0, $count = 10, $hide_auto_friend = null, $hide_no_avatar = null)
	{
		// 未指定参数时, 加载系统配置
		$config = model('Xdata')->lget('top_follower');
		!isset($hide_auto_friend) && $hide_auto_friend = intval($config['hide_auto_friend']);
		...
		if (($top_org = S($cache_id)) === false) {
			...
			$limit += $hide_no_avatar ? $count * 3 : $count;
               ...
               */       if($utype=="org"){
                             $where = ' WHERE `type` = 0 and fid in(select uid from ts_user where deputyoriid IS NOT NULL and deputyoriid <>"") ';
                         } else {
                             $where = ' WHERE `type` = 0 and fid in(select uid from ts_user where deputyoriid IS NULL)';
                         } 
			...
			if (!empty($res)) { // 过滤
				...
				//如果全都是无头像的.那就补充上去
				if(empty($top_org) && !empty($noPic))
					$top_org = $noPic;
			}
			
			//截取需要的前$count个
			if(is_array($top_org))
				$top_org = array_slice($top_org, 0, $count);
...
//					$list = $this->where("uid={$uid} AND type={$type} ")->order('follow_id DESC')->findpage($limit);
					...
					$list = $this->Table("{$this->tablePrefix}{$this->tableName} AS follow LEFT JOIN {$this->tablePrefix}user AS u ON follow.fid = u.uid LEFT JOIN {$this->tablePrefix}weibo_follow_group_link AS link ON link.follow_id = follow.follow_id")
								 ->field('follow.*')
								 ->where("u.uid is not null AND follow.uid={$uid} AND follow.type={$type} AND link.follow_id IS NULL ")
					...
					$list = $this->field('follow.*')
							 ->table("{$this->tablePrefix}{$this->tableName} AS follow LEFT JOIN {$this->tablePrefix}user AS u ON follow.fid = u.uid RIGHT JOIN {$this->tablePrefix}weibo_follow_group_link AS link ON link.follow_id=follow.follow_id AND link.uid=follow.uid")
							 ->where("follow.type={$type} AND follow.uid={$uid} AND link.follow_group_id={$gid} AND u.uid is not null")
							 ->order('follow.uid DESC')
							 ->findPage($limit);
				}
			}else{
				//$list = $this->where("uid=$uid AND type=$type")->order('follow_id DESC')->findpage($limit);
				$list = $this->Table("{$this->tablePrefix}{$this->tableName} AS wf LEFT JOIN {$this->tablePrefix}user AS u ON wf.fid = u.uid")
							 ->field('wf.*')
							 ->where("wf.uid=$uid AND wf.type=$type AND u.uid is not null")->order('wf.follow_id DESC')->findpage($limit);
			}
		}else{ //粉丝
			//$list = $this->where("fid=$uid AND type=$type")->order('follow_id DESC')->findpage($limit);
			$list = $this->Table("{$this->tablePrefix}{$this->tableName} AS wf LEFT JOIN {$this->tablePrefix}user AS u ON wf.uid = u.uid")
						 ->field('wf.*')
						 ->where("wf.fid=$uid AND wf.type=$type AND u.uid is not null")->order('wf.follow_id DESC')->findpage($limit);
