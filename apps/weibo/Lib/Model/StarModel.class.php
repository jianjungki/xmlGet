...
class StarModel extends Model {
	...
	// 按分组获取专家列表
	public function getStarsByGroup($limit=6,$mid){
		if(empty($this->group_list)){
		...
		foreach($group_list as $k=>&$v){
			$v['user_list'] = $this->getStarsCache($v['star_group_id'],$limit,true,$mid);
			if(!empty($v['son_list']) && is_array($v['son_list'])){
				foreach($v['son_list'] as $ks=>&$vs){
					$vs['user_list'] =  $this->getStarsCache($vs['star_group_id'],$limit,true,$mid);
	...
	public function getStarsCache($star_group_id,$limit=10, $refresh = false, $mid){
		...
		$cache = M()->query("SELECT u.*,wb.* FROM ts_weibo_star u LEFT JOIN (SELECT fid FROM ts_weibo_follow WHERE {$map_f} TYPE = 0)  
								wb ON wb.fid=u.uid WHERE u.uid<>1 AND {$map} GROUP BY star_id ORDER BY fid");
