...
class ClassifyModel extends Model
{
	...
	//归类微博
    function claWeibo( $id ,$from_id ,$title ,$work_type ,$app_type ,$uid ){
    		
    	$res='';	
    	switch ($app_type) {
			case 2:
				$res = M('message_content')->field('content')->where("message_id={$id}")->select();
				break;
			
			default:
				$res = M('weibo')->field('content')->where("weibo_id={$id}")->select();
				break;
		}
		// if (is_array($res)) {
			// $res = getSubByKey($res, 'content');
		// }
