...
class WeiboModel extends Model{
	 ...
	 */
     function publish($uid, $data, $from=0, $type=0, $type_data, $sync, $from_data){
     	...
     	if($id = $this->doSaveWeibo($uid, $data, $from , $type ,$type_data, $from_data)){  
     	    if($_POST['friendsId']!=''){
     	        $friendarr = explode(",", $_POST['friendsId']);
                $this->notifyToAtme($uid, $id, $data['content'],$friendarr);
     	    }else{
     	        $this->notifyToAtme($uid, $id, $data['content']);
     	    }
            if($_POST['orgfriendsId']!=''){
                $friendarr = explode(",", $_POST['orgfriendsId']);
                $this->notifyToAtme($uid, $id, $data['content'],$friendarr);
            }else{
                $this->notifyToAtme($uid, $id, $data['content']);
            }
            if($_POST['groupsId']!=''){
                $Group = D("GroupWeibo","group");

                $grouparr = explode(",", $_POST['groupsId']);
                foreach ($grouparr as $key => $value) {
                    $data['gid'] = $value;
                    $Group->doSaveWeibo($uid, $data, $from, $type, $type_data, $sync, $from_data);
                }
            }
            if($_POST['Toguest']!=''){
                $Interview = D("weibo_problem");
                $GuestArr = explode(",", $_POST['Toguest']);
                //添加信息
                $info_user['topics_id'] = t($_POST['topics_id']);
                $info_user['weibo_id'] = $id;
                $info_user['user_id'] = $uid;
                
                $info_guest = $info_user;
                //用户附加信息
                $info_user['content'] = $data['content'];
                $info_user['content_time'] = time();
                $info_user['content_type'] = 0;
                $Interview->data($info_user)->add();
                foreach ($GuestArr as $key => $value) {
                    $info_guest['guest_id'] = $value ;
                    $info_guest['content_type'] = 1;
                    $Interview->data($info_guest)->add();
                }
                //提示嘉宾
                $this->notifyToAtme($uid, $id, $data['content'],$GuestArr);
            }
     		return $id;
     	}else{
     		return false;
    ...
    function notifyToAtme($uid,$id,$content,$transpond_uid,$addCount=true){
    	...
    	$arrUids= array();
	//添加提到的Id
	    if( !is_array($transpond_uid)){
            array_push($arrUids,$transpond_uid);
        }
    	if( is_array($transpond_uid)){
    		$arrUids = $transpond_uid;
    	}
