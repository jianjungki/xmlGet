...
class OperateAction extends Action{
    ...
    function addcomment(){
        ...
        $info_user['weibo_id'] = $post['weibo_id'];
        if(M('weibo_problem')->where($info_user)->find()){
            $infodata['content'] = $post['content'];
            $infodata['guest_time'] = time();
            $map['weibo_id'] = $post['weibo_id'];
            $res = M('weibo_problem')->where($map)->setField("reply_time",time());
            $map['content_type'] = 1;
            $map['guest_id'] = $this->mid;
            $res = M('weibo_problem')->data($infodata)->where($map)->save();
        }
	...
	//把完成工作转回待办理
    function backToLater(){
		$map['app_id'] = intval(($_POST['weibo_id']));
		$map['app_type'] = intval(($_POST['app_type']));
		$res1 = M('work_finishtask')->field('task_id,title,content,work_type,uid,start_date,app_type,app_id')
		->where($map)->select();
		array_shift($res1[0]);
		$res2 = M('work_task')->where($map)->select();
		
		
		if (!empty($res2[0])) {
			$data['start_date'] = time();	
			$data['end_date'] = '';	
			$data['work_type'] = intval(($_POST['work_type']));	
			$flag = M('work_task')->data($data)->where($map)->save();
			if ($flag) {
				echo '1';
			}
			
		}elseif(!empty($res1[0])) {
			$res1[0]['start_date'] = time();
			$res1[0]['work_type'] = intval(($_POST['work_type']));	
			$flag = M('work_task')->data($res1[0])->add();
			if ($flag) {
				M('work_finishtask')->where($map)->delete();	
				dump(M('work_finnishtask'));
				echo '1';
			}
		}else {
			echo '0';
    ...
    function finishedWork(){
		...
		$map['app_type'] = intval(($_POST['app_type']));
		$res1 = M('work_task')->where($map)->select();
		array_shift($res1[0]);
		$res2 = M('work_finishtask')->where($map)->select();
		// dump($data);
		// // $res1['now_end_date'] = time();
		// if (empty($data)) {
			// echo "0";
		// }else {
			// M('work_finishtask')->data($data[0])->add();
			// echo '1';
		// }
		// display('publish');
			
		if (!empty($res2[0])) {
			$data['start_date'] = $res1[0]['start_date'];	
			$data['end_date'] = $res1[0]['end_date'];	
			$data['now_end_date'] = time();
			$res = M('work_finishtask')->data($data)->where($map)->save();
			if ($res) {
			echo '1';
		}
		}elseif(!empty($res1[0])) {
			$res1[0]['now_end_date'] = time();
			$flag = M('work_finishtask')->data($res1[0])->add();
			if ($flag) {
				M('work_task')->where($map)->delete();	
				echo '1';
			}
	...
	{
		$data['isdel'] = 1;
		$del_flag = intval( $_POST['del_flag'] );
		$map['task_id'] = intval( $_POST['id'] );
		$model_name = 'work_task';
		if ($del_flag) {
			$model_name = 'work_finishtask';
		}
		$res = M($model_name)->data($data)->where($map)->save();
		if ($res) {
			echo '1';
		}
	}
	...
	{
		$data['work_type'] = intval( $_POST['work_type'] );
		$res = M('work_task')->data($data)->where( 'task_id='.intval(($_POST['id'])) )->save();
		if ($res) {
			echo '1';
		}
	}
	...
	//分类
    function classify(){
    	$flag = D('Classify')->claWeibo( intval( $_POST['id'] ),intval( $_POST['uid'] ),$_POST['title'],intval( $_POST['work_type'] ),intval( $_POST['app_type'] ), $this->mid );
    	if( $flag ){
    		echo '1';
       	}
