...
class UserModel extends Model {
	...
	public function getUserIdByName($uname){
		...
		$name = $this->where("uname='".$uname."'")->getField('uid');
		return $name;
	}
	 ...
	 * @return array
	 */
    public function getUserList($map = '', $show_dept = false, $show_user_group = false, $field = '*', $order = 'uid ASC', $limit = 9) {
    	// $res = $this->where($map)->field($field)->order($order)->group("is_active")->count();
//        $res = $this->select();
           if($_GET['ordertype']=='follower')$order = "B.followerCount DESC";
           else if($_GET['ordertype']=='weibo')$order = "A.wbCount DESC";
           else $order = "A.".$order;
        //$res  = $this->where($map)->field($field)->order($order)->findPage($limit);
        $res  = M('')->table('
                    (SELECT u.*,COUNT(wb.uid) AS wbCount FROM ts_user u LEFT JOIN ts_weibo wb ON wb.uid=u.uid GROUP BY u.uid) A
                        LEFT JOIN
                    (SELECT u.uid,COUNT(wf.uid) AS `followerCount` FROM ts_user u LEFT JOIN ts_weibo_follow wf ON u.uid = wf.fid GROUP BY u.uid) B
                    ON A.uid = B.uid')->where($map)->order($order)->findPage($limit);
//        $User = M("User"); 
//        $res = $User->returnResult($res, "object");
//        $data2 = model("Adv")->returnResult($res,"object");
//        dump($data2);
...
//            dump($res);
	 ...
	 * @return array
	 */
	public function getAllUserList($map = '', $field = '*', $order = 'uid DESC', $limit = 16) {
		$res  = $this->where($map)->field($field)->order($order)->findPage($limit);
		return $res;	
	}
	 ...
	 * @return void
	 */
	public function doRecommend( $map,$act ){
			
		if( empty($map) ){
			throw new ThinkException( "不允许空条件操作数据库" );
		}
		switch( $act ){
			case "isExpert":   //推荐到广场领域专家风采
				$data['isExpert'] = 1;
				
			break;
			case "cancelExpert":   //取消推荐到广场领域专家风采
                $data['isExpert'] = 0;
				
			break;
			case "toOrgIndex":   //推荐到组织首页
				$data['toOrgIndex'] = 1;
				
			break;
			case "cancelOrgIndex":   //取消推荐组织首页
                $data['toOrgIndex'] = 0;
				
			break;
			case "isGov":   //推荐到广场行业协会
				$data['isGov'] = 1;
				
			break;
			case "cancelGov":   //取消推荐到广场行业协会
                $data['isGov'] = 0;
				
			break;
			case "isHot":   //推荐到广场组织风采
				$data['isHot'] = 1;
				
			break;
			case "cancelHot":   //取消推荐到广场组织风采
                $data['isHot'] = 0;
