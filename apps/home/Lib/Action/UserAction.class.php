...
class UserAction extends Action {
    ...
    function index() {
		...
		$data = $this->__paramUserFollowGroup($index_type);
		$data['type'] = in_array($_GET['type'], array('atme', 'work', 'collection')) ? $_GET['type'] : '0';
		switch ($data ['type']) {
			case 'atme':
			...
			case 'work':
				...
				// $mywork = $_GET['my_work']=='later' ? $_GET['my_work'] : 'today';
				$mywork = in_array($_GET['my_work'], array('later','finish')) ? $_GET['my_work'] : 'today';
				
				$data ['list'] = D ( 'Operate', 'weibo' )->getWork ( $this->mid , $mywork);
				$data ['my_work'] = $mywork;
		        // 同步的设置
		        $bind = M ( 'login' )->where ( 'uid=' . $this->mid )->findAll ();
		        foreach ( $bind as $v ) {
		...
		//是否已经归类
		$work_list = M("work_task")->where('app_type = 1 AND uid='.$this->mid)->select();
		$work_wid = getSubByKey($work_list, 'app_id');
		foreach( $data['list']['data'] as $key=>$value){
			// if ($value['uid']==$this->mid) {
	...
	//组织首页
	function orgindex() {
		// $org = D('User')->getUserList();
		// 组织推荐
		$map = 'deputyoriid is not null';
		$data['new_org'] = D('User')->getAllUserList($map, '*', 'ctime DESC', 20);
		$this->assign ( $data );
		$this->display ('orgindex');
	}
    ...
    function comments1() {
		...
		$uid = $this->mid;
		// $res = M('weibo_comment')->field("comment_id,'weibo' AS commentstype")
    					 ...
    					 // ->order('c.comment_id DESC')->select();
    					 // ->union("SELECT id as comment_id, TYPE AS commentstype FROM ts_comment WHERE uid=1 AND (appuid={$uid} OR to_uid={$uid})",true)->select();
    	
    	$sql = "SELECT  comment_id,'weibo' AS commentstype,c.cTime FROM ts_weibo_comment AS c LEFT JOIN ts_weibo AS w  ON c.weibo_id=w.weibo_id
				WHERE(c.isdel=0 AND w.isdel=0 AND (c.reply_uid=1 OR w.uid=1) AND c.uid != 1)
				UNION ALL
				SELECT  id, TYPE,cTime FROM ts_comment WHERE uid=1 AND (appuid='1' OR to_uid='1')   ORDER BY cTime DESC";
		$res['list'] = D('')->findPageBySql($sql,30,10);
		
		//微博评论
		$res1 ['type'] == 'receive' && model ( 'UserCount' )->setZero ( $this->mid, 'comment' );

        //$data['person'] = (in_array( $_GET['person'] , array('all','follow','other')) )?$_GET['person']:'all';
        $res1 ['person'] = 'all';
        $res1 ['list'] = D ( 'Comment', 'weibo' )->getCommentList ( $res1 ['type'], $res1 ['person'], $this->mid );
        
        //其他应用评论
        $dao = model ( 'GlobalComment' );
        $res2 ['type'] == 'receive' && $dao->setUnreadCountToZero ( $this->mid );

        $res2 ['person'] = 'all';
        $res2 ['list'] = $dao->getCommentList ( $res2 ['type'], $this->mid );

        /*
         * 缓存评论发表者, 被回复的用户,
         */
        // $ids = getSubBeKeyArray ( $res2 ['list'] ['data'], 'appuid,uid,to_uid' );
        // D ( 'User', 'home' )->setUserObjectCache ( array_unique ( array_merge ( $ids ['appuid'], $ids ['uid'], $ids ['to_uid'] ) ) );

        foreach ( $res2 ['list'] ['data'] as $k => $v ){
            $res2 ['list'] ['data'] [$k] ['data'] = unserialize ( $v ['data'] );
            }
		
		//合并评论
/*		foreach ( $res ['list'] ['data'] as $k => $v ){
            $type = $res ['list'] ['data'] [$k] ['commentstype'];
			$id = $res ['list'] ['data'] [$k] ['comment_id'];
			if ($type == 'weibo') {
				foreach ( $res1 ['list'] ['data'] as $k1 => $v1 ){
					if ($v1['comment_id'] == $id) {
						$res ['list'] ['data'] [$k] = $v1;
					}
				}
			}else{
				foreach ( $res2 ['list'] ['data'] as $k2 => $v2 ){
					if ($v2['id'] == $id) {
						$res ['list'] ['data'] [$k] = $v2;
					}
				}
        ...
        $data['total'] = $res['list'];
		$data['weibo'] = $res1['list'];
		$data['other'] = $res2['list'];
    ...
    function totalcomments() {
        $res ['type'] = ($_GET ['type'] == 'send') ? 'send' : 'receive';
        $res ['from_app'] = ($_GET ['from_app'] == 'other') ? 'other' : 'weibo';
    	...
    	$sql = "SELECT  comment_id,'weibo' AS commentstype,c.cTime,c.weibo_id AS appid,c.uid,c.content,c.`reply_comment_id`,'' AS data FROM ts_weibo_comment AS c LEFT JOIN ts_weibo AS w  ON c.weibo_id=w.weibo_id
WHERE(c.isdel=0 AND w.isdel=0 AND (c.reply_uid=1 OR w.uid=1) AND c.uid != 1)
UNION ALL
SELECT  id, TYPE,cTime,appid,uid,comment,toId AS reply_comment_id,data FROM ts_comment WHERE uid=1 AND (appuid='1' OR to_uid='1')   ORDER BY cTime DESC";
		$res['list'] = D('')->findPageBySql($sql,30,10);
		foreach ( $res ['list'] ['data'] as $k => $v ){
				if ($v['commentstype'] != 'weibo') {
					$res ['list'] ['data'] [$k] ['data'] = unserialize ( $v ['data'] );
				}else{
					$res ['list']['data'][$k]['mini'] = D('Weibo', 'weibo')->getOneLocation($v['appid'], '', false);
					// dump(D('Weibo', 'weibo'));
		    		// if( !$value['reply_comment_id'] ){
		    			// $res ['list']['data'][$key]['reply_uid']  = $list['data'][$key]['mini']['uid'];
		    			// $res ['list']['data'][$key]['ismini'] = true;
		    		// }else {
		    			// $res ['list']['data'][$key]['comment'] = $this->getCommentDetail($value['reply_comment_id']);
		    		// }
				}
        	}        
		$this->assign ( $res );
        $this->setTitle ( $res ['type'] == 'receive' ? L('receive_comment') : L('send_comment') );
        $this->display ();
		}
    ...
    function comments() {
        ...
        //} else {
            ...
            $data ['person'] = 'all';
            $data ['list2'] = $dao->getCommentList ( $data ['type'], $this->mid );

            /*
             * 缓存评论发表者, 被回复的用户,
             */
            $ids = getSubBeKeyArray ( $data ['list2'] ['data'], 'appuid,uid,to_uid' );
            D ( 'User', 'home' )->setUserObjectCache ( array_unique ( array_merge ( $ids ['appuid'], $ids ['uid'], $ids ['to_uid'] ) ) );

            foreach ( $data ['list2'] ['data'] as $k => $v )
                $data ['list2'] ['data'] [$k] ['data'] = unserialize ( $v ['data'] );
        //}
...
//      $data['list']['count'] = D ( 'Operate', 'weibo' )->where("content LIKE '%#{$data['topics']['name']}#%' AND isdel=0")->count();
    ...
    //推荐组织
    public function searchRecomuser(){
        $this->__checkSearchPopedom ();
        $data ['search_key'] = $this->__getSearchKey ();
        Session::pause();
        $data['hot_org']  = M('')->field("u.*,wb.isdel,count(wb.uid) as wb")
                                ->table("ts_user u left join ts_weibo wb on wb.uid=u.uid")
                                ->where("deputyoriid IS NOT NULL and (isdel=0 or isdel IS NULL) and u.uid<>1 group by uname order by count(wb.uid) desc")
                                ->findPage(20);
        
        $data['new_org'] = D ( 'Follow', 'weibo' )->doSearchUser ( $data ['search_key'] );
        $this->assign ( $data );
        $this->display ("orgindex");
    }
    
    public function ChangeOrgUser(){
            $limit = $_POST['limit'];
            $num = $_POST['count']*$limit;
            $html  = '';
            $recommend = S('_recommend_orgindex_cache');
            ...
            for ($i = $num; $i < $alllimit+$num; $i++) {
                ...
                $html .=     '</a>';
                $html .=     '<a class="userface" target="_blank" href="'.U('home/Space/index',array('uid'=>$shifted_user['uid'])).'" rel="face" uid="'.$shifted_user['uid'].'">';
                $html .=         '<span>'.getShort(getUserName($shifted_user['uid']),8,'...').'</span>';
                $html .=     '</a>';
                $html .= '</li>';
            }
            echo $html;
     ...
     * @time 2012-11-30
     */
     public function getQuanziName(){
         $uids = $_POST['uidstr'];
         $uids = is_array($uids)?$uids:explode(",",$uids);
         foreach ($uids as $key => $value) {
             $getstr .= " @".getGroupName($value);
         }
         echo $getstr;
     }
