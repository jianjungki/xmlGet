...
class SelectFriendsAction extends Action {
        ...
        public function getOne() {
				...
				if($name){
					...
					$followings = $Model->field('follow.fid AS fuid,user.uname AS funame')
										...
										->where("follow.uid={$this->mid} AND user.uname LIKE '%{$name}%' AND user.deputyoriid is NULL")
					...
					$followers  = $Model->field('follow.uid AS fuid,user.uname AS funame')
										...
										->where("follow.fid={$this->mid} AND user.uname LIKE '%{$name}%'  AND user.deputyoriid is NULL")
										->order('follow.follow_id DESC')
										->limit(10)
										->findAll();
				}
        ...
        public function getAll() {
				...
				if($typeId==2){
					$follow = M('')->field('follow.fid AS fuid,user.uname AS funame')
								   ->table("{$db_prefix}weibo_follow AS follow LEFT JOIN {$db_prefix}user AS user ON follow.fid=user.uid")
								   ->where("follow.uid={$this->mid} AND user.deputyoriid is NULL")
								   ->order('follow.follow_id DESC')
								   ->findPage(12);				
				}elseif($typeId==3){
					$follow = M('')->field('follow.uid AS fuid,user.uname AS funame')
								   ->table("{$db_prefix}weibo_follow AS follow LEFT JOIN {$db_prefix}user AS user ON follow.uid=user.uid")
								   ->where("follow.fid={$this->mid} AND user.deputyoriid is NULL")
								   ->order('follow.follow_id DESC')
								   ->findPage(12);
				}else{//默认显示互粉
					$follow = M('')->field('follow.fid AS fuid,user.uname AS funame')
								   ->table("{$db_prefix}weibo_follow AS follow LEFT JOIN {$db_prefix}user AS user ON follow.fid=user.uid")
								   ->where("follow.uid={$this->mid} AND follow.fid IN (SELECT uid FROM {$db_prefix}weibo_follow WHERE fid={$this->mid}) AND user.deputyoriid is NULL")
								   ->order('follow.follow_id DESC')
								   ->findPage(12);
				}
        ...
        public function getCount() {
                ...
                $db_prefix  =  C('DB_PREFIX');
				if($typeId==2){
					$followNum = M('')->field('follow.fid AS fuid,user.uname AS funame')->table("{$db_prefix}weibo_follow AS follow LEFT JOIN {$db_prefix}user AS user ON follow.fid=user.uid")->where("follow.uid={$this->mid}  AND user.deputyoriid is NULL")->order('follow.follow_id DESC')->count();				
				}elseif($typeId==3){
					$followNum = M('')->field('follow.uid AS fuid,user.uname AS funame')->table("{$db_prefix}weibo_follow AS follow LEFT JOIN {$db_prefix}user AS user ON follow.uid=user.uid")->where("follow.fid={$this->mid} AND user.deputyoriid is NULL")->order('follow.follow_id DESC')->count();				
				}else{//默认显示互粉
					$followNum = M('')->field('follow.fid AS fuid,user.uname AS funame')->table("{$db_prefix}weibo_follow AS follow LEFT JOIN {$db_prefix}user AS user ON follow.fid=user.uid")->where("follow.uid={$this->mid} AND follow.fid IN (SELECT uid FROM {$db_prefix}weibo_follow WHERE fid={$this->mid}) AND user.deputyoriid is NULL")->order('follow.follow_id DESC')->count();
				}
				echo $followNum;
        }
