<?php
class SelectMembersAction extends Action {
	    var $groupmember;
        function _initialize(){
            $groupmember = D("user")->select();
        }
        //------------------------------------------------以下是选择好友组件相关-----------------------------------
        ...
        public function getAll() {
				...
				if($typeId==2){
					$follow = M('')->field('follow.fid AS fuid,user.uname AS funame')
								   ->table("{$db_prefix}weibo_follow AS follow LEFT JOIN {$db_prefix}user AS user ON follow.fid=user.uid")
								   ->where("follow.uid={$this->mid} AND user.uid NOT IN (SELECT uid FROM {$db_prefix}org_user WHERE orgid={$this->orgid}) ")
								   ->order('follow.follow_id DESC')
								   ->findPage(12);				
				}elseif($typeId==3){
					$follow = M('')->field('follow.uid AS fuid,user.uname AS funame')
								   ->table("{$db_prefix}weibo_follow AS follow LEFT JOIN {$db_prefix}user AS user ON follow.uid=user.uid")
								   ->where("follow.fid={$this->mid} AND user.uid NOT IN (SELECT uid FROM {$db_prefix}org_user WHERE orgid={$this->orgid})  ")
								   ->order('follow.follow_id DESC')
								   ->findPage(12);
				}else{//默认显示互粉
					$follow = M('')->field('follow.fid AS fuid,user.uname AS funame')
								   ->table("{$db_prefix}weibo_follow AS follow LEFT JOIN {$db_prefix}user AS user ON follow.fid=user.uid")
								   ->where("follow.uid={$this->mid} AND follow.fid IN (SELECT uid FROM {$db_prefix}weibo_follow WHERE fid={$this->mid}) AND user.uid NOT IN (SELECT uid FROM {$db_prefix}org_user WHERE orgid={$this->orgid})")
								   ->order('follow.follow_id DESC')
								   ->findPage(12);
				}
