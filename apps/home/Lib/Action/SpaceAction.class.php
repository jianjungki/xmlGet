...
class SpaceAction extends Action
{
	function _initialize() {
		...
		if ('detail' != ACTION_NAME) {
            ...
            $Orguser = D("OrgUser")->where("orgid = ".$user_info['deputyoriid'])->find();
	   	   	if ($user_info) {
				if(!is_numeric($user_info['deputyoriid'])){
					$userinfo = array(
	                    ...
	                    $userinfo = array(
	                                '组织全称' => $Orguser['unitFullName'],
									'微博地址' => U('home/Space/index', array('uid' => $user_info['domain'] ? $user_info['domain'] : $this->uid)),								
									'所在地'  => $user_info['location'],
									'所属行业'    => $Orguser['category'],
								);
				 }
    			// 基本信息-钩子
    ...
    public function index()
    {
        ...
        //获取公告信息
        $stateinfo = M('OrgStatement')->where("orgid = ".$data['user']['deputyoriid'])->order("postdate desc")->find();
        $this->assign('stateinfo',$stateinfo);
        
        $orgset = D("OrgUser")->where("orgid =".$data['user']['deputyoriid'])->find();
        $this->assign("websiteurl",$orgset['website']);
        //是否已经归类
    ...
    public function profile()
    {
        ...
        if($data['userInfo']['detail']['deputyoriid']){
            $usertype = "org";
            $pOrgUserProfile = D('OrgUserProfile');
            $pOrgUserProfile->uid = $this->uid;
            $data['OrguserInfo']  = $pOrgUserProfile->getUserInfo(true);
        }else{
            $usertype = "user";
        }
        ...
        $userdata = D('User')->getUserByIdentifier($this->uid);
...
//    	$this->_canViewSpace();
//    	// 关注的人列表
//    	$data['list'] = D('Follow','weibo')->getList($this->uid,'following',0,$data['gid']);
//            dump($data['list']);
//    	$this->assign($data);
//    	$this->setTitle(getUserName($this->uid) . '的朋友');
//    	$this->display();
//
//    }
    ...
    public function showSpaceCard(){
    	...
    	if($uid){
            ...
            $data['canView']   =   canView( $this->mid,$uid);
     ...
     * */
     ...
     {
         $res = D("UserRemark")->addRemark($this->mid,$_REQUEST['uid'],$_REQUEST['remark']);
