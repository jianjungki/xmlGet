...
class EventModel extends BaseModel{
	...
	public function appendContent( $data ){
		...
		$data['isBigcover']  = $opts_list['isBigcover'];
	...
	public function doAddEvent($eventMap,$optsMap,$cover){
		...
		$eventMap['bigcoverId']    = $cover['status']?$cover['info'][1]['id']:0;
...
function get_photo_url($savepath) {
	...
	public function doIsHot( $map,$act ){
		...
		switch( $act ){
			...
			case "cancel":   //取消推荐
                ...
                $data['rTime'] = 0;
			break;
			case "recommendBig":   //推荐到大图
				$data['isBigcover'] = 1;
                $data['rTime'] = time();
			break;
			case "cancelBig":   //取消推荐大图
                $data['isBigcover'] = 0;
                $data['rTime'] = 0;
     ...
     * @return void
     */
    public function getBigcoverList(){
    	$opts_ids  = self::factoryModel( 'opts' )->field('id')->where('isBigcover=1')->limit(5)->findAll();
    	foreach( $opts_ids as &$v ){
    		$v = $v['id'];
    	}
    	$event_map['optsId'] = array('in',$opts_ids);
    	...
    	foreach($event_ids as &$v){
    		$v['type']    = $typeDao->getTypeName($v['type']);
            $v['address'] = getShort($v['address'],6);
            $v['bigcoverId'] = getCover($v['bigcoverId'],740,220);
    	}
    	return $event_ids;
    }
