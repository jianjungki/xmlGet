...
class IndexAction extends Action {
    ...
    public function _initialize() {
		...
		//读取大封面活动列表
        $is_bigcover_list = $this->event->getBigcoverList();
        $this->assign('is_bigcover_list',$is_bigcover_list);
        // 活动分类
    ...
    public function doEditEvent() {
		...
		$old_cover['info'][1]['id'] = intval( $_POST['old_bigcover'] );
		// dump($old_cover);
        if( $stime > $etime ){
		...
		if ($_FILES['cover']['size']>0) {
			$temp_cover = X('Xattach')->upload('event',$options);
			if (count($temp_cover['info'])==2) {
				$cover = $temp_cover;
			}else {
				$cover['info'][0]=$temp_cover['info'][0];
			}
		}elseif ($_FILES['bigcover']['size']>0) {
			$temp_bigcover = X('Xattach')->upload('event',$options);
			$cover['info'][1]=$temp_bigcover['info'][0];
		}
    ...
    public function eventPhoto() {
        ...
        if($result = $this->event->getEventContent( $id,$uid )) {
			...
			//获取相册信息
			$album = D('Album','photo');
			// $map['cTime'] = $this->event->getField('cTime',"id={$id}");
			// $map['userId'] = $uid;
			// $map['name'] = $album->getField('name',"cTime={$cTime}");
			$map['albumtype'] = 1;
			$map['appId'] =  $id;
			$album = $album->field('id')->where($map)->find();
			$result['aid'] = $album['id'];
            $this->assign($result);
            // $this->assign('event', $result);
            $this->setTitle($result['title'].'相册');
            $this->display();
        }else {
            $this->assign('jumpUrl',U('Event/Index/index'));
            $this->error( '错误的访问页面，请检查链接' );
	...
	public function sendMessage() {
		...
		$eid = $_POST['eid'];
		$uid = $_POST['uid'];
		$tid = D('EventUser')->getField('uid',"id={$id} AND eventId={$eid}");		
				
		$title = $this->event->getField('title',"id={$eid} AND uid={$this->mid}");
		$data['to'] = $tid;
		$data['title'] = '“' . $title . '”活动参加申请通过';
		$url = '<a href="' . U('event/Index/eventDetail', array('id'=>$eid,'uid'=>$uid)).'" target="_blank">快来看看</a>';
		$data['content'] = '你申请参加的活动“' . $title . '”的审核已经通过，' . $url;
     ...
     */
     public function changeCount() {
     	$id = $_POST['eventId'];	
		$flag = $_POST['flag'];
		$event = $this->event->where( 'id ='.$id )->find();
		// dump($event);
		if ($flag=='join') {
			echo $event['joinCount'];
		} elseif ($flag=='att') {
			echo $event['attentionCount'];
		}
     }
