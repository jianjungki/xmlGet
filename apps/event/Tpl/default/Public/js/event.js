      ...
      if( text == 1 ){
    	...
    	if( action == 'joinIn' ){
    		if( allow == 1 ){
    			...
    			$('.list_joinIn_'+id).html('<a href="javascript:EventDelAction( '+id+',null,\'joinIn\' );add('+id+',\'join\');">取消参加</a>');
    			$('.detail_joinIn_'+id).html('<button class="btn_w" style="margin-right:5px;" onclick="javascript:EventDelAction( '+id+',null,\'joinIn\' );add('+id+',\'join\');">取消参加</button>');
    		}
    	}else if( action == 'attention' ){
    		$('.list_attention_'+id).html('<a href="javascript:EventDelAction( '+id+',null,\'attention\');add('+id+',\'att\');">取消关注</a>');
    		$('.detail_attention_'+id).html('<button class="btn_w" style="margin-right:5px;" onclick="javascript:EventDelAction( '+id+',null,\'attention\');add('+id+',\'att\');">取消关注</button>');
    	}
      }else if( text == -2 ){
        if( allow == 1 ){
      ...
      if( text == 1 ){
    	...
    	if( action == 'joinIn' ){
    		$('.list_joinIn_'+id).html(
    				'<a href="javascript:EventAction( '+id+','+allow+',\'joinIn\' );add('+id+',\'join\');">我要参加</a>'+
    				'<span class="list_attention_'+id+'">'+
    				'<a href="javascript:EventAction( '+id+',null,\'attention\');add('+id+',\'att\');">我要关注</a>'+
                    '</span>'
    		);
    		$('.detail_joinIn_'+id).html(
    				'<button class="btn_b" style="margin-right:5px;" onclick="javascript:EventAction( '+id+','+allow+',\'joinIn\' );add('+id+',\'join\');">我要参加</button>'+
    				'<span class="detail_attention_'+id+'">'+
    				'<button class="btn_b" style="margin-right:5px;" onclick="javascript:EventAction( '+id+',null,\'attention\');add('+id+',\'att\');">我要关注</button>'+
                    '</span>'
    		);
    	}else if( action == 'attention' ){
        	$('.list_attention_'+id).html('<a href="javascript:EventAction( '+id+',null,\'attention\');add('+id+',\'att\');">我要关注</a>');
        	$('.detail_attention_'+id).html('<button class="btn_b" style="margin-right:5px;" onclick="javascript:EventAction( '+id+',null,\'attention\');add('+id+',\'att\');">我要关注</button>');
    	}
      }else if( text == -2 ){
...
function agree( id,eventId,uid ){
  $.post( U('event/Index/doAgreeAction'),{id:id,eventId:eventId,uid:uid},function( text ){
      if( text == -5){
    	...
    	sendMessage(id,eventId,uid);
    	ui.success( '操作成功！通知已经、通过私信发送给该成员' );
...
//活动参加、关注人数刷新
function add (eventId,flag) {
	setTimeout(function() {
	$.post( U('event/Index/changeCount'),{eventId:eventId,flag:flag},function( res ){
	  	if(res>=0){
	  		// alert('res='+res);
	  		// alert('flag='+flag);
	  		switch(flag){
	  			...
	  			case 'join':
	  				var oldCount = $('#join_'+eventId).html();
	  				// if(oldCount == res){
	  					// ui.success( '需要发起人审核后才能参加该活动' );
	  				// }else{
	  					// alert('oldcount='+oldCount);
	  					// alert('id='+'#join_'+eventId);
						$('#join_'+eventId).html(res);
	  					// $('#join_'+eventId).wrap('<span id="join_'+evebtId+'">'+res+'</span>');
...
//成员审核后发私信通知
function sendMessage(id,eventId,uid){
	$.post( U('event/Index/sendMessage'),{id:id,eid:eventId,uid:uid},function(txt){
		txt = eval( '(' + txt + ')' );
		$.post(U('home/Message/doPost'), {to:txt.to, title:txt.title, content:txt.content}, function(res){
			if ( res == 1 ) {
